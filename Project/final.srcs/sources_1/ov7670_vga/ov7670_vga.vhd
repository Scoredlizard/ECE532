...
entity ov7670_vga is
    port ( 
        ...
        switch_clr  : in  STD_LOGIC_VECTOR(1 downto 0); -- 2-bit input signal to control VGA output behavior
        clk25       : in  STD_LOGIC;
   ...
   begin
      if rising_edge(clk25) then
         ...
         if blank = '0' then
            case switch_clr is
                when "00" =>
                    -- Regular color output
                    vga_red   <= frame_pixel(11 downto 8);
                    vga_green <= frame_pixel(7 downto 4);
                    vga_blue  <= frame_pixel(3 downto 0);
                when "01" =>
                    -- All channels set to red value
                    vga_red   <= frame_pixel(11 downto 8);
                    vga_green <= frame_pixel(11 downto 8);
                    vga_blue  <= frame_pixel(11 downto 8);
                when "10" =>
                    -- All channels set to green value
                    vga_red   <= frame_pixel(7 downto 4);
                    vga_green <= frame_pixel(7 downto 4);
                    vga_blue  <= frame_pixel(7 downto 4);
                when "11" =>
                    -- All channels set to blue value
                    vga_red   <= frame_pixel(3 downto 0);
                    vga_green <= frame_pixel(3 downto 0);
                    vga_blue  <= frame_pixel(3 downto 0);
            end case;
            
            if address(17 downto 4) = x"256B" then
                vga_red   <= "1"&"1"&"1"&"1";
                vga_green <= "1"&"1"&"1"&"1";
                vga_blue  <= "1"&"1"&"1"&"1";
             end if;
            
            --vga_red   <= frame_pixel( 8 downto 6) & "0";
            --vga_green <= frame_pixel( 5 downto 3) & "0";
            --vga_blue  <= frame_pixel( 2 downto 0) & "0";
         else
            vga_red   <= (others => '0');
            vga_green <= (others => '0');
            vga_blue  <= (others => '0');
